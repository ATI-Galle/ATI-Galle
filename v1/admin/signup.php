...
// MODIFIED: Fetch departments (courses) for the dropdown
$departments = [];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // ADDED: Validate Role and Department
    if (empty($_POST["role"])) {
        ...
        if ($role === 'sub_admin') {
            ...
            $cid = NULL; // Super admins are not tied to a department
        } else {
    ...
    if (empty($username_err) && empty($password_err) && empty($confirm_password_err) && empty($role_err) && empty($cid_err) && empty($profile_image_err)) {
        ...
        if ($stmt = mysqli_prepare($conn, $sql)) {
            ...
            $param_role = $role; // ADDED
            $param_cid = $cid;   // ADDED
            $param_status = 0; // Default status for new users is inactive
...
<head>
    ...
    <style>
        ...
        .wrapper {
            ...
            width: 500px; /* Increased width for new fields */
            max-width: 95%;
        }
...
<body>
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data">
            ...
            <!-- ADDED: Role and Department Selection -->
            <div class="form-group">
    ...
    <!-- ADDED: JavaScript to show/hide department dropdown -->
    <script>
