...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($username_err) && empty($password_err)) {
        // MODIFIED: Prepare a select statement to fetch user details including status, role, and cid
        ...
        if ($stmt = mysqli_prepare($conn, $sql)) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                if (mysqli_stmt_num_rows($stmt) == 1) {
                    // MODIFIED: Bind result variables, including the status, role, and cid
                    ...
                    if (mysqli_stmt_fetch($stmt)) {
                        ...
                        if ($status == 1) {
                            if (password_verify($password, $hashed_password)) {
                                ...
                                // ADDED: Store the user's role and department ID in the session
                                $_SESSION["role"] = $role; // e.g., 'super_admin' or 'sub_admin'
                                $_SESSION["cid"] = $cid;   // e.g., 'HNDIT' or NULL for super_admin
