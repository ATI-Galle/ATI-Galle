...
// --- Authorization ---
// Check if the user has management permissions based on role or cid
$has_management_access = (isset($_SESSION['role']) && $_SESSION['role'] === 'super_admin') || (isset($_SESSION['cid']) && $_SESSION['cid'] === 'SAdmin');
...
// --- Authorization --- Only process POST requests if the user has management access
if ($has_management_access && $_SERVER['REQUEST_METHOD'] == 'POST') {
    $action = $_POST['action'] ?? '';
...
// --- Fetch Existing Tenders (Visible to all users) ---
$tenders = [];
...
<hr>
<?php endif; // End of management access check for the form ?>
    ...
    <table class="data-table">
        ...
        <tbody>
            <?php if (empty($tenders)): ?>
                <tr><td colspan="<?php echo $has_management_access ? '6' : '5'; ?>" style="text-align: center;">No tenders found.</td></tr>
            <?php else: ?>
...
document.addEventListener('DOMContentLoaded', function() {
    // --- Authorization --- Check if the user has access before enabling JS for the form
    const hasManagementAccess = <?php echo json_encode($has_management_access); ?>;
        ...
        document.querySelectorAll('.edit-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                if (data.closing_date) {
                    ...
                    // This format is required by the datetime-local input
