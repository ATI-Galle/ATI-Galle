...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($username_err) && empty($password_err) && empty($confirm_password_err) && empty($pet_name_err) && empty($childhood_nickname_err) && empty($profile_image_err)) {
        ...
        $sql = "INSERT INTO users (username, password, profile_image, pet_name_hint, childhood_nickname_hint) VALUES (?, ?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($conn, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssss", $param_username, $param_password, $param_profile_image, $param_pet_name, $param_childhood_nickname);
...
<head>
    ...
    <style>
        ...
        .wrapper{ width: 400px; padding: 20px; margin: auto; margin-top: 30px; } /* Adjusted width for more fields */
    </style>
</head>
<body>
    <div class="wrapper">
